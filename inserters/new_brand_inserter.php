...
$sql = null;
foreach (glob("../pre_made/*/new_brand.html") as $filename) {
  ...
  foreach($brandRows as $key => $row){
    $subject = preg_replace('/\'/ms', '[[RSQUO]]', $brandRows[3]);
    $preHeader = preg_replace('/\'/ms', '[[RSQUO]]', $brandRows[4]);
    $voucher = '0';
  }
